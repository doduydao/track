...
def run(hits, nt, M, model_path_out, solution_path_out):
    ...
    print("K=", K)
    # create_variables
    phi, c = create_variables(model, hits)
                ...
                for j in range(1, n_p_2):
                    for p_3 in range(p_2 + 1, K + 2):
                        ...
                        for k in range(1, n_p_3):
                            ...
                            count_constraint += 4
                            model.add_constraint(phi[p_1, p_2, i, j] <= c[p_2, j])
                            model.add_constraint(phi[p_2, p_3, j, k] <= c[p_2, j])
                            model.add_constraint(phi[p_1, p_2, i, j] + phi[p_2, p_3, j, k] - c[p_2, j] <= 1)
                            model.add_constraint(c[p_2, j] >= 0)

                            # model.add_constraint(
                            #     beta*((2 - phi[p_1, p_2, i, j] - phi[p_2, p_3, j, k])**2) <= c[p_2, j], ctname=c7)
    ...
    model.add_constraint(objective >= 0, ctname="LB")
...
if __name__ == '__main__':
    ...
    layers = sorted(list(hits.keys()))
    # print(layers)
    model_path_out = "result_f2_Lacomme_2/model_docplex.lp"
    solution_path_out = "result_f2_Lacomme_2/solution.json"
    ...
    out = "result_f2_Lacomme_2/result.PNG"
